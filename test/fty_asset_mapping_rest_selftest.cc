...
static test_item_t all_tests[] = {
    ...
    {"stub_asset_mapping_rest", stub_asset_mapping_rest_test, true, true, NULL}, {NULL, NULL, 0, 0, NULL} //  Sentinel
};
...
{
    test_item_t* item;
    printf("Running fty-asset-mapping-rest selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq(item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test(verbose);
#ifdef FTY_ASSET_MAPPING_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_asset_mapping_rest_private_selftest(verbose, item->subtest);
#endif // FTY_ASSET_MAPPING_REST_BUILD_DRAFT_API
    }
...
int main(int argc, char** argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        if (streq(argv[argn], "--help") || streq(argv[argn], "-h")) {
            puts("fty_asset_mapping_rest_selftest.c [options] ...");
    ...
    if (test) {
        printf("Running fty-asset-mapping-rest test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test(verbose);
#ifdef FTY_ASSET_MAPPING_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_asset_mapping_rest_private_selftest(verbose, test->subtest);
#endif // FTY_ASSET_MAPPING_REST_BUILD_DRAFT_API
    } else
        test_runall(verbose);
